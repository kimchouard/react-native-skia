...
The steps listed for Webpack holds true for Metro. The way you would resolve `fs` and `path` though is different.

Add this JS script to your `postinstall` routine, in order to avoid canvaskit throwing an error:
```js
...
If you're using [Expo Router](https://docs.expo.dev/router/introduction/), you can not defer the root component registration, so you'd have to use the [code-splitting](#using-code-splitting).
